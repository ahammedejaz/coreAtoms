...
function InlineReviewForm({ productId, orderId, productName, onDone }) {
  ...
  return (
    <div className="mt-3 space-y-3 rounded-2xl border border-line bg-bone p-4">
      <p className="text-xs font-semibold text-stone-600">Rate your purchase: <span className="text-ink">{productName}</span></p>
      <div className="flex items-center gap-1">
        {[1, 2, 3, 4, 5].map((i) => (
          <button key={i} type="button" onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(0)} onClick={() => setRating(i)}
            className="text-2xl leading-none transition-transform hover:scale-110" aria-label={`${i} star${i !== 1 ? "s" : ""}`}>
            <span className={i <= (hovered || rating) ? "text-amber" : "text-line-strong"} aria-hidden="true">★</span>
          </button>
        ))}
        {(hovered || rating) > 0 && (
...
function InlineReplacementForm({ orderId, userId, onDone }) {
  ...
  return (
    <div className="mt-4 space-y-3 rounded-2xl border border-line bg-bone p-4">
      <p className="text-sm font-semibold text-ink">Request a replacement</p>
            ...
            {previews.map((src, i) => (
              <div key={i} className="relative h-20 w-20 rounded-xl border border-[#E8E4DE] overflow-hidden group">
                ...
                <button
                  ...
                  className="absolute right-0.5 top-0.5 grid h-5 w-5 place-items-center rounded-full bg-ink/70 text-white opacity-0 transition-opacity group-hover:opacity-100"
...
const STATUS_STYLES = {
  placed: "bg-brand-soft text-brand border border-navy-200",
  processing: "bg-brand-soft text-brand border border-navy-200",
  confirmed: "bg-brand-soft text-brand border border-navy-200",
  shipped: "bg-brand text-white border border-brand",
  out_for_delivery: "bg-brand text-white border border-brand",
...
export default function MyOrders() {
  ...
  return (
    <div>
      ...
      {corecoinsEnabled && (
        <ScrollReveal>
          <div className="mb-6 rounded-panel border border-amber-200 bg-amber-soft/60 px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-amber-deep">Your CoreCoins</p>
              ...
              <div className="ml-auto text-right">
                <p className="text-[11.5px] text-stone-600">Earn coins on every purchase.</p>
                <p className="text-[11.5px] text-stone-600">Redeem them at checkout.</p>
              </div>
            </div>
          </div>
        </ScrollReveal>
      ...
      <div className="mb-6 rounded-panel border border-line bg-white p-4 sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-3 flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
              {STATUS_OPTIONS.map((s) => (
                <button key={s} type="button" onClick={() => setStatusFilter(s)}
                  className={`shrink-0 rounded-full border px-3.5 py-1.5 text-xs font-semibold capitalize transition-colors duration-150 ${statusFilter === s
                    ? "border-ink bg-ink text-white"
                    : "border-line-strong bg-white text-stone-600 hover:border-ink hover:text-ink"
                    }`}
                >
                  {s === "all" ? "All orders" : s}
                </button>
            ...
            <div className="relative group shrink-0">
              <div className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none">
                <Search className="h-4 w-4 text-stone-400 transition-colors group-focus-within:text-brand" strokeWidth={1.75} aria-hidden="true" />
              </div>
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search orders…"
                className="h-10 w-full rounded-full border border-line-strong bg-white pl-10 pr-4 text-sm text-ink placeholder:text-stone-400 outline-none transition-[border-color,box-shadow] duration-150 hover:border-ink focus:border-brand focus:shadow-[0_0_0_4px_rgba(30,58,95,0.08)] sm:w-52"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 self-start sm:self-auto shrink-0">
            ...
            <div className="inline-flex items-center rounded-full bg-bone px-3 py-1.5">
      ...
      {!loading && loadError && (
        <div className="border-t border-line py-16 text-center">
          ...
          <p className="font-display text-2xl font-semibold tracking-tight text-ink">Couldn't load your orders</p>
      ...
      {!loading && !loadError && filtered.length === 0 && (
        <div className="border-t border-line py-20 text-center">
          <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-bone text-brand">
            <PackageOpen className="h-6 w-6" strokeWidth={1.5} aria-hidden="true" />
          </span>
          <p className="mt-5 font-display text-2xl font-semibold tracking-tight text-ink">No orders yet</p>
        ...
        {paginated.map((o) => {
          ...
          return (
            <ScrollReveal key={o.id}>
              <div key={o.id} className="panel p-5 sm:p-6">
                {/* Order header */}
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="mb-1 text-xs text-stone-500">Order</p>
                    <p className="font-display text-base font-semibold tracking-tight text-ink tabular-nums">{String(o.id).slice(0, 8).toUpperCase()}</p>
                ...
                {status === "payment_failed" && (
                  <div className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 flex items-start gap-3">
                    <TriangleAlert className="mt-0.5 h-4 w-4 shrink-0 text-red-600" strokeWidth={1.75} aria-hidden="true" />
                      ...
                      <div className="mt-4 border-t border-[#E8E4DE] pt-4">
                        <div className="rounded-2xl border border-line bg-bone p-4">
                          <div className="mb-1 flex items-center gap-2">
                            <RefreshCw className="h-4 w-4 text-stone-400" strokeWidth={1.75} aria-hidden="true" />
                            <span className="text-xs font-semibold text-ink">Replacement request</span>
                          ...
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              {isDelivered && it.product_id && (
                                alreadyReviewed
                                  ? <p className="mt-0.5 inline-flex items-center gap-1 text-[11px] font-medium text-emerald-700"><Check className="h-3 w-3" strokeWidth={2.5} aria-hidden="true" />Reviewed</p>
      ...
      {filtered.length > PAGE_SIZE && (
        <div className="mt-6 flex items-center justify-between">
          <button type="button" onClick={() => { setPage(p => Math.max(1, p - 1)); window.scrollTo(0, 0); }} disabled={safePage <= 1}
            className="btn-secondary btn-sm disabled:opacity-30 disabled:cursor-not-allowed"><ChevronLeft className="h-4 w-4" strokeWidth={1.75} aria-hidden="true" />Previous</button>
          ...
          <button type="button" onClick={() => { setPage(p => Math.min(totalPages, p + 1)); window.scrollTo(0, 0); }} disabled={safePage >= totalPages}
            className="btn-secondary btn-sm disabled:opacity-30 disabled:cursor-not-allowed">Next<ChevronRight className="h-4 w-4" strokeWidth={1.75} aria-hidden="true" /></button>
        </div>
      )}
    </div>
  );
