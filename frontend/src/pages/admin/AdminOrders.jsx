...
export default function AdminOrders({ onOrdersChange, isActive = true }) {
    ...
    // Tracks "orderId_status" keys — persisted to Supabase wa_notifications table
    const [waSentKeys, setWaSentKeys] = useState(new Set());

    // Load existing notification records when orders are loaded
    useEffect(() => {
        ...
        (async () => {
            ...
            if (data && data.length) {
                setWaSentKeys(new Set(data.map((r) => `${r.order_id}_${r.status}`)));
            }
        })();
    }, [orders]);

    const markWaSent = async (order, currentStatus) => {
        // Optimistically update UI
        setWaSentKeys((prev) => new Set(prev).add(`${order.id}_${currentStatus}`));

        // Persist to Supabase
        const { data: { user } } = await supabase.auth.getUser();
        const phone = (order.shipping_phone || "").replace(/[^\d+]/g, "");
        const customerName = order.shipping_name || order.user_full_name || "";

        const { error } = await supabase.from("wa_notifications").upsert({
            order_id: order.id,
        ...
        if (error) {
            showToast("Failed to save WhatsApp status", "error");
            // Revert optimistic update
            setWaSentKeys((prev) => {
                const next = new Set(prev);
                next.delete(`${order.id}_${currentStatus}`);
                return next;
            });
        }
    };

    const isWaSent = (orderId, status) => waSentKeys.has(`${orderId}_${status}`);

    const SITE_URL = "coreatoms.in";

    const WA_MESSAGES = {
        placed: (name, id) =>
            `Hello ${name}!\n\nThank you for choosing *Core Atoms*!\n\nWe're delighted to confirm that your order *#${id}* has been *successfully placed*.\n\nOur team will begin processing it shortly and keep you updated every step of the way.\n\n*Track your order anytime:*\nhttps://${SITE_URL}/orders\n\nIf you have any questions, feel free to reply to this message - we're always happy to help!\n\nWarm regards,\n*Team Core Atoms*`,
        processing: (name, id) =>
                                        ...
                                        <div key={o.id} className={["rounded-2xl border p-4", selectedOrderIds.has(o.id) ? "border-blue-300 bg-blue-50/50" : "border-[#E8E4DE] bg-white"].join(" ")}>
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="flex items-start gap-2 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        <div className="text-sm font-semibold text-stone-900 break-all">#{String(o.id).slice(0, 8)}</div>
                                            ...
                                            <div className="mt-3 flex items-center gap-2">
                                                <select
                                                    ...
                                                    className={`h-10 min-w-0 flex-1 rounded-full border border-line-strong bg-white pl-3.5 pr-8 text-[13px] font-medium text-ink ${!canEditStatus(o) ? "opacity-50 cursor-not-allowed" : ""}`}
                                                ...
                                                <button
                                                    ...
                                                    className={`inline-flex h-10 shrink-0 items-center gap-1.5 rounded-full px-3.5 text-[12.5px] font-semibold transition-[transform,background-color] duration-150 active:scale-[0.96] ${isOpen ? "bg-ink text-white" : "border border-line-strong bg-white text-ink"}`}
                                                >
                                                    Details
                                                    <svg className={`h-3.5 w-3.5 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                                                </button>

                                                {/* WhatsApp notification */}
                                                {(() => {
                                                    const waUrl = getWhatsAppUrl(o);
                                                    const sent = isWaSent(o.id, st);
                                                    const waIcon = <svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>;
                                                    if (!waUrl) return null;
                                                    return sent ? (
                                                        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-bone text-stone-400" title="WhatsApp message sent" aria-label="WhatsApp message sent">
                                                            <svg className="h-4 w-4 text-[#25D366]" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                                                        </span>
                                                    ) : (
                                                        <a
                                                            href={waUrl}
                                                            ...
                                                            onClick={() => markWaSent(o, st)}
                                                            aria-label="Message the customer on WhatsApp"
                                                            className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#25D366] text-white transition-transform duration-150 active:scale-[0.94]"
                                                        >
                                                            {waIcon}
                                                        </a>
                                                    );
                                                            ...
                                                            <div className="flex flex-col gap-2">
                                                                ...
                                                                <div className="flex gap-2">
                                                                    <button
                                                                        type="button"
                                                                        onClick={() => toggleOrderExpanded(o.id)}
                                                                        className="flex-1 rounded-lg bg-[#1e3a5f] px-2 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-[#162d4a] active:scale-95 transition-all duration-150"
                                                                    >
                                                                        {expandedOrderIds.has(o.id) ? "Hide" : "Details"}
                                                                    </button>
                                                                    ...
                                                                    {(() => {
                                                                        ...
                                                                        return waUrl ? (
                                                                            sent ? (
                                                                                <div className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-stone-100 px-2 py-1.5 text-xs font-semibold text-stone-400">
                                                                                    <svg className="h-3.5 w-3.5 text-[#25D366]" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                                                                                    Sent
                                                                                </div>
                                                                            ) : (
                                                                                <a
                                                                                    ...
                                                                                    rel="noopener noreferrer"
                                                                                    onClick={() => markWaSent(o, st)}
                                                                                    className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-[#25D366] px-2 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-[#1ebe5b] active:scale-95 transition-all duration-150"
                                                                                >
